...
class A2C(ActorCriticRLModel):
    ...
    def my_init_learn(self, total_timesteps, callback=None, log_interval=100, tb_log_name="A2C",
                  reset_num_timesteps=True):
        new_tb_log = self._init_num_timesteps(reset_num_timesteps)
        self.mycallback = self._init_callback(callback)

        SetVerbosity(self.verbose)
        tbw =TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name,new_tb_log)
        self.my_writer = tbw.__enter__()
        self._setup_learn()
        self.learning_rate_schedule = Scheduler(initial_value=self.learning_rate, n_values=total_timesteps,
                                                schedule=self.lr_schedule)

        self.my_t_start = time.time()
        self.mycallback.on_training_start(locals(), globals())
        self.my_log_interval=log_interval
    def my_step_learn(self, update):
        self.mycallback.on_rollout_start()
        # true_reward is the reward without discount
        rollout = self.runner.run(self.mycallback)
        # unpack
        obs, states, rewards, masks, actions, values, ep_infos, true_reward = rollout

        self.mycallback.on_rollout_end()

        # Early stopping due to the callback
        if not self.runner.continue_training:
            self.my_finish_learn()
            return

        self.ep_info_buf.extend(ep_infos)
        _, value_loss, policy_entropy = self._train_step(obs, states, rewards, masks, actions, values,
                                                         self.num_timesteps // self.n_batch, self.my_writer)
        n_seconds = time.time() - self.my_t_start
        fps = int((update * self.n_batch) / n_seconds)

        if self.my_writer is not None:
            total_episode_reward_logger(self.episode_reward,
                                        true_reward.reshape((self.n_envs, self.n_steps)),
                                        masks.reshape((self.n_envs, self.n_steps)),
                                        self.my_writer, self.num_timesteps)

        if self.verbose >= 1 and (update % self.my_log_interval == 0 or update == 1):
            explained_var = explained_variance(values, rewards)
            logger.record_tabular("nupdates", update)
            logger.record_tabular("total_timesteps", self.num_timesteps)
            logger.record_tabular("fps", fps)
            ...
            if len(self.ep_info_buf) > 0 and len(self.ep_info_buf[0]) > 0:
                ...
                logger.logkv('ep_len_mean', safe_mean([ep_info['l'] for ep_info in self.ep_info_buf]))
            logger.dump_tabular()
        return obs, states, rewards, masks, actions, values, ep_infos, true_reward
    def my_finish_learn(self):
        self.mycallback.on_training_end()
        return self
    def my_learn(self, total_timesteps, callback=None, log_interval=100, tb_log_name="A2C",
                  reset_num_timesteps=True):
